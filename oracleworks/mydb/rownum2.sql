...
-- 재귀복사(INSERT INTO ~ SELECT) : VALUES 사용 안함
...
-- FROM 절 서브쿼리 사용 - 인라인뷰
-- 왜 rn을 사용하는가? ROWNUM은 1이 있어야 검색이 되므로 rn이라는 변수(별칭) 사용
SELECT * 
FROM (SELECT ROWNUM rn, drink_code, drink_name FROM drink2)
-- WHERE ROWNUM >= 11 AND ROWNUM <= 20; -- ROWNUM 사용 - 검색 안 됨
WHERE rn >= 11 AND rn <= 20; -- 별칭(변수) 사용시 검색됨
